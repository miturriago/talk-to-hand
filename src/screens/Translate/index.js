...
import {examplePlugin} from '../../frame-processors/index';
...
function CameraView({navigation}) {
  ...
  const frameProcessor = useFrameProcessor(frame => {
    'worklet';
    console.log('Return frame:', frame);
    const labels = labelImage(frame);
    console.log(`Return Values: ${JSON.stringify(labels)}`);   
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={{backgroundColor: 'red', marginTop: 5}}>
        <Text style={{color: 'white'}}>{info.label}</Text>
      </View>
